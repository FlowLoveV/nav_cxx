...
struct Result {
  ...
  bool operator==(const Err<E> &val) const { return is_err() && unwrap_err() == val.value; }
  ...
  Result or_else(Function op) {
    if (is_err()) return op(unwrap_err());
    return Ok(ok().value());
  }
  ...
  bool contains_err(const E &this_value) { return is_err() ? unwrap_err() == this_value : false; }
...
struct Ok {
  ...
  template <typename Function>
  Result<T, T> and_then(Function op) {
...
struct Err {
  ...
  template <typename Function>
  Result<E, E> and_then(Function op) {
