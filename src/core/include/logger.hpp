...
#define nav_log(level, ...)                                                        \
  nav::details::globalFormattedLogger->log(level, "{}", nav::sourceInformation()); \
  nav::details::globalPureLogger->log(level, __VA_ARGS__);
...
#define nav_data_export(...) nav::details::globalDataLogger->debug(__VA_ARGS__);
...
std::shared_ptr<spdlog::logger> createPureSpdlogger(
    const std::string& loggerFileName, const std::string& loggerName,
    const std::string& direction = constants::logDirection);
