...
static int is_navmsg(int msg) {
  return msg == 1019 || msg == 1020 || msg == 1044 || msg == 1045 ||
         msg == 1046 || msg == 1042 || msg == 63 || msg == 1041;
}
...
static void raw2rtcm(rtcm_t *out, const raw_t *raw, int ret) {
  ...
  if (ret == 1) {
    ...
    if (sys == SYS_GLO) {
      ...
      out->nav.eph[sat - 1 + MAXSAT * set] =
          raw->nav.eph[sat - 1 + MAXSAT * set];
      out->ephsat = sat;
      out->ephset = set;
    }
  } else if (ret == 5) {
...
static void write_rtcm3_msm(stream_t *str, rtcm_t *out, int msg, int sync) {
  ...
  int i, j, n, ns, sys, nobs, code, nsat = 0, nsig = 0, nmsg,
                                    mask[MAXCODE] = {0};
