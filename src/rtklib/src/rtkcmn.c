 ...
 *atmosphere delay at radio wavelengths, Journal of geophysical research, 1996
...
extern int getcodepri(int sys, uint8_t code, const char *opt) {
  ...
  return (p = strchr(codepris[i][j], obs[1])) ? 14 - (int)(p - codepris[i][j])
                                              : 0;
}
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    if (!strncmp(buff, "IONUTC", 6)) {
      ...
      sscanf(buff, "IONUTC,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf",
             ...
             &nav->utc_gps[1], &nav->utc_gps[2], &nav->utc_gps[3],
             &nav->utc_gps[4]);
      continue;
    }
    if ((p = strchr(buff, ',')))
...
extern int rtk_uncompress(const char *file, char *uncfile) {
  int stat = 0;
  char *p, cmd[64 + 2048] = "", tmpfile[1024] = "", buff[1024], *fname,
                    *dir = "";
