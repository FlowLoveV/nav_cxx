...
#include "better_enum/enum.h"  //BETTER_ENUM
...
typedef enum { CODE, PHAS, NUM_MEAS_TYPES } E_MeasType;
...
BETTER_ENUM(E_Block, short int, UNKNOWN, GPS_I, GPS_II, GPS_IIA, GPS_IIR_A, GPS_IIR_B, GPS_IIR_M,
            GPS_IIF, GPS_IIIA, GLO_M, GLO, GLO_K1A, GLO_K1B, GLO_K2,
            GLO_MP,  // GLO-M+
            GAL_0A, GAL_0B, GAL_1, GAL_2, BDS_2M, BDS_2G, BDS_2I, BDS_3SI_SECM, BDS_3SM_CAST,
            BDS_3SI_CAST, BDS_3SM_SECM, BDS_3M_CAST, BDS_3M_SECM_A, BDS_3G, BDS_3I, BDS_3M_SECM_B,
            QZS_1, QZS_2I, QZS_2G, QZS_2A, IRS_1I, IRS_1G, IRS_2G)

BETTER_ENUM(E_OffsetType, short int, UNSPECIFIED, APC, COM)

BETTER_ENUM(E_TrigType, short int, COS, SIN)

BETTER_ENUM(E_EmpAxis, short int, NONE, D, Y, B, R, T, N, P, Q)

BETTER_ENUM(KF, short int, NONE, ONE,

            REC_POS, REC_POS_RATE, STRAIN_RATE,
            ...
            REC_CLOCK, REC_SYS_BIAS = REC_CLOCK, REC_CLOCK_RATE, REC_SYS_BIAS_RATE = REC_CLOCK_RATE,
            REC_CLOCK_RATE_GM, REC_SYS_BIAS_RATE_GM = REC_CLOCK_RATE_GM,

            SAT_CLOCK, SAT_CLOCK_RATE, SAT_CLOCK_RATE_GM,

            TROP, TROP_GRAD, TROP_MODEL,

            IONOSPHERIC, IONO_STEC, REC_PCO_X, REC_PCO_Y, REC_PCO_Z, SAT_PCO_X, SAT_PCO_Y,
            SAT_PCO_Z,

            REC_PCV,

            EOP_ADJUST, EOP_RATE_ADJUST,

            CALC,

            SLR_REC_RANGE_BIAS, SLR_REC_TIME_BIAS,

            XFORM_XLATE, XFORM_RTATE, XFORM_SCALE, XFORM_DELAY,

            AMBIGUITY, CODE_BIAS, PHASE_BIAS,
            ...
            BEGIN_ORBIT_STATES, ORBIT, EMP_D_0, EMP_D_1, EMP_D_2, EMP_D_3, EMP_D_4,

            EMP_Y_0, EMP_Y_1, EMP_Y_2, EMP_Y_3, EMP_Y_4,

            EMP_B_0, EMP_B_1, EMP_B_2, EMP_B_3, EMP_B_4,

            EMP_R_0, EMP_R_1, EMP_R_2, EMP_R_3, EMP_R_4,

            EMP_T_0, EMP_T_1, EMP_T_2, EMP_T_3, EMP_T_4,

            EMP_N_0, EMP_N_1, EMP_N_2, EMP_N_3, EMP_N_4,

            EMP_P_0, EMP_P_1, EMP_P_2, EMP_P_3, EMP_P_4,

            EMP_Q_0, EMP_Q_1, EMP_Q_2, EMP_Q_3, EMP_Q_4, END_ORBIT_STATES,

            BEGIN_INERTIAL_STATES, GYRO_BIAS, GYRO_SCALE, ACCL_BIAS, ACCL_SCALE, IMU_OFFSET,
            END_INERTIAL_STATES,
...
BETTER_ENUM(E_ChiSqMode, int, NONE, INNOVATION, MEASUREMENT, STATE)
...
BETTER_ENUM(E_Period, int, SECOND = 1, MINUTE = 60, HOUR = 60 * 60, DAY = 60 * 60 * 24,
            WEEK = 60 * 60 * 24 * 7, YEAR = 60 * 60 * 24 * 365,

            SECONDS = SECOND, MINUTES = MINUTE, HOURS = HOUR, DAYS = DAY, WEEKS = WEEK,
            YEARS = YEAR, SEC = SECOND, MIN = MINUTE, HR = HOUR, DY = DAY, WK = WEEK, YR = YEAR,
            ...
            SQRT_YR = YEAR, SQRT_SECS = SECOND, SQRT_MINS = MINUTE, SQRT_HRS = HOUR, SQRT_DYS = DAY,
            SQRT_WKS = WEEK, SQRT_YRS = YEAR, SQRT_SECOND = SECOND, SQRT_MINUTE = MINUTE,
            SQRT_HOUR = HOUR, SQRT_DAY = DAY, SQRT_WEEK = WEEK, SQRT_YEAR = YEAR,
            SQRT_SECONDS = SECOND, SQRT_MINUTES = MINUTE, SQRT_HOURS = HOUR, SQRT_DAYS = DAY,
            SQRT_WEEKS = WEEK, SQRT_YEARS = YEAR)
...
BETTER_ENUM(E_Inverter, int, NONE, INV, LLT, LDLT, COLPIVHQR, BDCSVD, JACOBISVD, FULLPIVLU,
            FIRST_UNSUPPORTED = FULLPIVLU, FULLPIVHQR)

BETTER_ENUM(E_MongoType, int, NONE, STATES, RESIDUALS, TRACE, LIST)
...
BETTER_ENUM(E_ObsCode2, int, NONE, P1, P2, C1, C2, C3, C4, C5, C6, C7, C8, L1, L2, L3, L4, L5, L6,
            L7, L8, LA)

BETTER_ENUM(E_ARmode, short int, OFF, ROUND, ITER_RND, BOOTST, LAMBDA, LAMBDA_ALT, LAMBDA_AL2,
            LAMBDA_BIE)

BETTER_ENUM(
    E_NavRecType, short int,
    NONE,  ///< NONE for unknown */
...
BETTER_ENUM(E_NavMsgType, short int,
            ...
            CNVX)  ///< GPS/QZSS CNAV or CNAV-2 Navigation Message  BeiDou-3 CNAV-1, CNAV-2 or
                   ///< CNAV-3 Navigation Message

BETTER_ENUM(E_SatType, short int, NONE, GEO, IGSO, MEO)

BETTER_ENUM(E_StoCode, short int, NONE, GPUT, GLUT, GLGP, GAUT, GAGP,
            GPGA = GAGP,  // From RINEX 3.04 the GPGA label is replaced by GAGP, while the value and
                          // sign for the Galileo minus GPS time offset remains unchanged.
            GAGL, BDUT, BDGP, BDGL, BDGA, QZUT, QZGP, QZGL, QZGA, QZBD, IRUT, IRGP, IRGL, IRGA,
            IRBD, IRQZ, SBUT, SBGP, SBGL, SBGA, SBBD, SBQZ, SBIR)

BETTER_ENUM(E_UtcId, short int, NONE, UTC_USNO, UTC_SU, UTCGAL, UTC_NTSC, UTC_NICT, UTC_NPLI,
            UTCIRN, UTC_OP, UTC_NIST)

BETTER_ENUM(E_SbasId, short int, NONE, WAAS, EGNOS, MSAS, GAGAN, SDCM, BDSBAS, KASS, A_SBAS, SPAN)

BETTER_ENUM(RtcmMessageType, uint16_t, NONE = 0,
            ...
            GAL_FNAV_EPHEMERIS = 1045, GAL_INAV_EPHEMERIS = 1046,

            GPS_SSR_ORB_CORR = 1057, GPS_SSR_CLK_CORR = 1058, GPS_SSR_CODE_BIAS = 1059,
            GPS_SSR_COMB_CORR = 1060, GPS_SSR_URA = 1061, GPS_SSR_HR_CLK_CORR = 1062,
            GPS_SSR_PHASE_BIAS = 1265,

            GLO_SSR_ORB_CORR = 1063, GLO_SSR_CLK_CORR = 1064, GLO_SSR_CODE_BIAS = 1065,
            GLO_SSR_COMB_CORR = 1066, GLO_SSR_URA = 1067, GLO_SSR_HR_CLK_CORR = 1068,
            GLO_SSR_PHASE_BIAS = 1266,

            MSM4_GPS = 1074, MSM5_GPS = 1075, MSM6_GPS = 1076, MSM7_GPS = 1077,

            MSM4_GLONASS = 1084, MSM5_GLONASS = 1085, MSM6_GLONASS = 1086, MSM7_GLONASS = 1087,

            MSM4_GALILEO = 1094, MSM5_GALILEO = 1095, MSM6_GALILEO = 1096, MSM7_GALILEO = 1097,

            MSM4_QZSS = 1114, MSM5_QZSS = 1115, MSM6_QZSS = 1116, MSM7_QZSS = 1117,

            MSM4_BEIDOU = 1124, MSM5_BEIDOU = 1125, MSM6_BEIDOU = 1126, MSM7_BEIDOU = 1127,

            // GLONASS_AUX_OPERATION_INFO = 1230

            GAL_SSR_ORB_CORR = 1240, GAL_SSR_CLK_CORR = 1241, GAL_SSR_CODE_BIAS = 1242,
            GAL_SSR_COMB_CORR = 1243, GAL_SSR_URA = 1244, GAL_SSR_HR_CLK_CORR = 1245,
            GAL_SSR_PHASE_BIAS = 1267,

            QZS_SSR_ORB_CORR = 1246, QZS_SSR_CLK_CORR = 1247, QZS_SSR_CODE_BIAS = 1248,
            QZS_SSR_COMB_CORR = 1249, QZS_SSR_URA = 1250, QZS_SSR_HR_CLK_CORR = 1251,
            QZS_SSR_PHASE_BIAS = 1268,

            SBS_SSR_ORB_CORR = 1252, SBS_SSR_CLK_CORR = 1253, SBS_SSR_CODE_BIAS = 1254,
            SBS_SSR_COMB_CORR = 1255, SBS_SSR_URA = 1256, SBS_SSR_HR_CLK_CORR = 1257,
            SBS_SSR_PHASE_BIAS = 1269,

            BDS_SSR_ORB_CORR = 1258, BDS_SSR_CLK_CORR = 1259, BDS_SSR_CODE_BIAS = 1260,
            BDS_SSR_COMB_CORR = 1261, BDS_SSR_URA = 1262, BDS_SSR_HR_CLK_CORR = 1263,
            BDS_SSR_PHASE_BIAS = 1270,

            COMPACT_SSR = 4073, IGS_SSR = 4076, CUSTOM = 4082)

BETTER_ENUM(CompactSSRSubtype, unsigned short, NONE = 0, MSK = 1, ORB = 2, CLK = 3, COD = 4,
            PHS = 5, BIA = 6, URA = 7, TEC = 8, GRD = 9, SRV = 10, CMB = 11, ATM = 12)

// the order and spacing of these is magic, dont modify
BETTER_ENUM(IgsSSRSubtype, unsigned short, NONE = 0,

            GROUP_ORB = 1, GROUP_CLK = 2, GROUP_CMB = 3, GROUP_HRC = 4, GROUP_COD = 5,
            GROUP_PHS = 6, GROUP_URA = 7, GROUP_ION = 8,

            GPS_OFFSET = 20, GPS_ORB = 21, GPS_CLK = 22, GPS_CMB = 23, GPS_HRC = 24, GPS_COD = 25,
            GPS_PHS = 26, GPS_URA = 27,

            GLO_OFFSET = 40, GLO_ORB = 41, GLO_CLK = 42, GLO_CMB = 43, GLO_HRC = 44, GLO_COD = 45,
            GLO_PHS = 46, GLO_URA = 47,

            GAL_OFFSET = 60, GAL_ORB = 61, GAL_CLK = 62, GAL_CMB = 63, GAL_HRC = 64, GAL_COD = 65,
            GAL_PHS = 66, GAL_URA = 67,

            QZS_OFFSET = 80, QZS_ORB = 81, QZS_CLK = 82, QZS_CMB = 83, QZS_HRC = 84, QZS_COD = 85,
            QZS_PHS = 86, QZS_URA = 87,

            BDS_OFFSET = 100, BDS_ORB = 101, BDS_CLK = 102, BDS_CMB = 103, BDS_HRC = 104,
            BDS_COD = 105, BDS_PHS = 106, BDS_URA = 107,

            SBS_OFFSET = 120, SBS_ORB = 121, SBS_CLK = 122, SBS_CMB = 123, SBS_HRC = 124,
            SBS_COD = 125, SBS_PHS = 126, SBS_URA = 127,

            IONVTEC = 201)

BETTER_ENUM(E_Source, short int, NONE, PRECISE, SSR, KALMAN, BROADCAST, NOMINAL, MODEL, REMOTE)
...
BETTER_ENUM(E_ThirdBody, short int, MERCURY = 1, VENUS = 2, EARTH = 3, MARS = 4, JUPITER = 5,
            SATURN = 6, URANUS = 7, NEPTUNE = 8, PLUTO = 9, MOON = 10,
...
BETTER_ENUM(E_MEASDataType, short int, NONE = 0, GYRO_Z = 5, WHEEL_FL = 6, WHEEL_FR = 7,
            WHEEL_RL = 8, WHEEL_RR = 9, SPEED_TICK = 10, SPEED = 11, GYRO_TEMP = 12, GYRO_Y = 13,
            GYRO_X = 14, ACCL_X = 16, ACCL_Y = 17, ACCL_Z = 18)
...
BETTER_ENUM(E_Component, short int, NONE, X, LLH, P, DX, PREFIT, POSTFIT, VARIANCE, OBSERVED,
            HEADING, RANGE, SAT_CLOCK, REC_CLOCK, SAGNAC, RELATIVITY1, RELATIVITY2,
            REC_ANTENNA_DELTA, REC_PCO, SAT_PCO, REC_PCV, SAT_PCV, TIDES_SOLID, TIDES_OTL,
            TIDES_ATL, TIDES_SPOLE, TIDES_OPOLE, IONOSPHERIC_COMPONENT, IONOSPHERIC_COMPONENT1,
            IONOSPHERIC_COMPONENT2, IONOSPHERIC_MODEL, PHASE_WIND_UP, PHASE_AMBIGUITY,
            REC_RANGE_BIAS, REC_TIME_BIAS, REC_PHASE_BIAS, SAT_PHASE_BIAS, REC_CODE_BIAS,
            SAT_CODE_BIAS, TROPOSPHERE, TROPOSPHERE_MODEL, EOP, NET_RESIDUAL, ORBIT_PT,
            SAT_REFLECTOR_DELTA, CENTRAL_FORCE, ALBEDO, INDIRECT_J2, EMPIRICAL, GENERAL_RELATIVITY,
            EGM, SRP_CANNONBALL, SRP_BOXWING, ANTENNA_THRUST, PLANETARY_PERTURBATION)

enum E_ReturnType { UNSUPPORTED, OK, WAIT, GOT_OBS, BAD_LENGTH };
