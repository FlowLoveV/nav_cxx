...
void compareOrbits(vector<string> files) {
  ...
  if (acsConfig.output_network_trace) {
    ...
    createNewTraceFile("Network", logptime, acsConfig.network_trace_filename,
                       traceDummy.traceFilename, true, acsConfig.output_config);
  }
  ...
  for (int i = 1; i < navVec.size(); i++) {
    ...
    for (auto& [time, pephMap0] : pephMapMap0) {
      ...
      for (auto& [id, peph0] : pephMap0) {
        ...
        if (peph1.vel.isZero()) {
          ...
          peph1.vel =
              (interpStopPos - interpStartPos) / (interpStopTime - interpStartTime).to_double();
        }
    ...
    for (auto& [key, vec] : transformStatisticsMap) {
      ...
      tracepdeex(0, trace, "^ %-10s %d averaged %+12.4e %-5s with std %12.4e over %d epochs\n",
                 KF::_from_integral(key.type)._to_string(), key.num, avg, key.comment.c_str(), std,
                 n);
    }
  }
}
  ...
  for (int i = 1; i < navVec.size(); i++) {
    ...
    for (auto& [id, attMap0] : nav0.attMapMap) {
      ...
      for (auto& [time, att0] : attMap0) {
        ...
        tracepdeex(0, std::cout, "\n%s - %s - %6.1fdeg", time.to_string(0).c_str(), id.c_str(),
                   angle);
      }
    }
  }
