...
struct Navigation {
  ...
  map<SatSys, map<E_NavMsgType, GMap<GTime, Eph, std::greater<GTime>>>>
      ephMap;  ///< GPS/QZS/GAL/BDS ephemeris
  map<SatSys, map<E_NavMsgType, GMap<GTime, Geph, std::greater<GTime>>>>
      gephMap;  ///< GLONASS ephemeris
  map<SatSys, map<E_NavMsgType, GMap<GTime, Seph, std::greater<GTime>>>>
      sephMap;  ///< SBAS ephemeris
  map<SatSys, map<E_NavMsgType, GMap<GTime, Ceph, std::greater<GTime>>>>
      cephMap;  ///< GPS/QZS/BDS CNVX ephemeris
  map<E_Sys, map<E_NavMsgType, GMap<GTime, ION, std::greater<GTime>>>> ionMap;  ///< ION messages
  map<E_StoCode, map<E_NavMsgType, GMap<GTime, STO, std::greater<GTime>>>>
      stoMap;                                                                   ///< STO messages
