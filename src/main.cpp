...
class CostAndJacobianCopyingCostFunction : public ceres::SizedCostFunction<1, 1, 1> {
 public:
  CostAndJacobianCopyingCostFunction(int index, const MyEvaluationCallback& evaluation_callback)
  ...
  bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const final {
    ...
    if (jacobians[0] != nullptr) jacobians[0][0] = evaluation_callback_.jacobians()(index_, 0);
    if (jacobians[1] != nullptr) jacobians[1][0] = evaluation_callback_.jacobians()(index_, 1);
    return true;
  }
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < kNumObservations; ++i) {
    problem.AddResidualBlock(new CostAndJacobianCopyingCostFunction(i, evaluation_callback), nullptr, &m, &c);
  }
